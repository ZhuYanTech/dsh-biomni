...
| `biomni/tool/*.py` | 20 个模块约 200 个领域函数，MIT 协议 | 在那个解释器里作为普通库复用 |
| `ToolRetriever` | 按 prompt 检索相关工具子集，因为 200+ 个 schema 塞不进上下文 | **不复现**——见下文 |
| `<execute>` / `<solution>` 标签 | 早于可靠 function calling 的文本协议 | **不复现**——dsh 有原生工具调用 |

**提示词里只列模块，不列函数。** 全部约 183 个函数放不进上下文，而且也不需要：解释器是持久的，模型花一次调用做 `dir()` + `inspect.signature()` 就能拿到准确签名，而且永不过期。这是对 Biomni 用检索层解决的同一个问题的一个更便宜的答案。实测的循环恰好是「猜 → introspect → 纠正」。
...
执行内核、设置页和环境探针可用并有测试覆盖。接下来是领域工具库的打磨，以及是否要做检索层的论证。
