...
Besides the tool modules, this environment may carry two other Biomni assets, each
with its own skill when there is anything real to offer:

  \`biomni-data-lake\`   biomedical datasets already downloaded on this machine
  \`biomni-software\`    bioinformatics packages and command-line tools installed here

Load \`biomni-data-lake\` before answering anything that could be settled with local
data rather than a web query — the files are already downloaded and cleaned, and the
skill carries their exact paths. Load \`biomni-software\` before hand-rolling an
analysis: a validated tool for it may already be installed. The command-line tools
there run through the \`bash\` tool, which is not restricted for them — only \`python\`
and \`pip\` are.

Both are generated by checking this machine, so they list only what is really here.
A dataset or tool absent from them is absent: say so rather than guessing a path or
substituting a web lookup for the dataset.`
